...
def grabcut_crop(image):
    img_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    img_dilated_1 = dilate(img_gray)
    img_blurred = cv2.medianBlur(img_dilated_1, 5)  # kernel_size = 7
    ...
    if all_contours is None:
        return image
    bounding_rect = cv2.boundingRect(merged_squares[0])
    bgdModel = np.zeros((1,65),np.float64)
    fgdModel = np.zeros((1,65),np.float64)
    try:
        mask, _, _ = cv2.grabCut(image, None, bounding_rect, bgdModel, fgdModel,5, cv2.GC_INIT_WITH_RECT)
...
def _crop(image_path):
    ...
    all_contours, squares, max_square_contour, merged_squares = get_contours(img_dilated_2)
    if all_contours is None:
        return orig_img, orig_img, img_gray, img_dilated_1, img_blurred, img_canny, img_dilated_2, all_contours, squares, \
...
def crop(image_path):
    output, _, _, _, _, _, _, _, _, _, _ = _crop(image_path)
    return output
